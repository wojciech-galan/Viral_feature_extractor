...
class SeqEntrySeq(BaseXML):
    ...
    def __init__(self, seq_entry_seq_, fasta_path):
        ...
        if self.representation not in ['raw', 'const', 'ref', 'consen',
                                       'delta']:  # [ 'raw', 'const', 'ref', 'consen', 'delta' ]
        ...
        if 'biosource_subtype' in biosource:  # nie zawsze jest
            ...
            for subsource in biosource_subtype.subsource:
                if subsource.subsource_subtype.value == 'segment':
                    self.subsource_subtype = (
                    subsource.subsource_subtype.value, subsource.subsource_subtype.text, subsource.subsource_name.text)
