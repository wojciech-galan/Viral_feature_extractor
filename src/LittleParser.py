...
class LittleParser(object):
	...
	def fromHandle(cls, handle):
		...
		if object_dict['_seq_strands'] and object_dict['_seq_strands']!='other':
		    ...
		    if "dsRNA" in lineage1 or "dsDNA" in lineage1:
...
def _parse(handle):
	...
	if 'seq_entry_seq_' in seq_entry_:
		...
		pdb.set_trace()
	#ssseq = None
	ssseq = SeqRepresentation( uniSeq )
	...
	for description in descriptions:
		...
		if not description.keys()[0] in ['Seqdesc_source', 'Seqdesc_pub', 'Seqdesc_user', 'Seqdesc_update-date', 'Seqdesc_comment', 'Seqdesc_title', 'Seqdesc_molinfo', 'Seqdesc_create-date', 'Seqdesc_embl', 'Seqdesc_genbank']:
	...
	if 'Bioseq-set_seq-set' in bioseq_set:
		...
		for seq_entry in seq_set:
		    molecule=seq_entry["Seq-entry_seq"]['Bioseq']['Bioseq_inst']['Seq-inst']['Seq-inst_mol'].attributes['value']
