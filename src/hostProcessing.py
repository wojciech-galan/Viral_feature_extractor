...
def putHostsInDb( hosts_dict, clear=False ):
	...
	if clear:
		daos.clear()
		species_list  = [ { 'tax_id': k, 'name':v[0], 'lineage':str( v[1].split( '; ' ) ) } for k, v in hosts_dict.iteritems() ]
...
def findHostInNCBITaxonomy( host_name ):
	...
	if len( id_list ) == 1:
		...
		return None # do poprawy
	return host_data[1] # to lineage

def findHostLineage( host_name ):
	...
	# teraz sprawdzamy, czy nazwa usera nie zaczyna się przypadkiem którąś z nazw z bazy
		...
		if ';' in host_name:
			#pdb.set_trace()
			return findHostLineage( host_name.split(';')[0].strip() )
	elif len( host_name.split() ) > 1 and ( host_name.endswith(' sp.') or host_name.endswith(' L.') ):
		#pdb.set_trace()
		return findHostLineage( host_name[:-3].strip() )
	else:
		#szukamy hosta w NCBI Taxonomy
		#pdb.set_trace()
		return findHostInNCBITaxonomy( host_name )
