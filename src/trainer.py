...
attribs = (
'gi', 'host', 'host_lineage', 'length', 'lineage', 'molecule', 'nuc_frequencies', 'relative_nuc_frequencies_one_strand',
...
def getAttributesClassesIdsFeatures(att_list, objects, classes_infecting, classes_systematics=None):
    ...
    if classes_infecting:
        ...
        for cls_num, cls_description in sorted(classes_infecting.iteritems(), key=lambda x: x[0]):
            ...
            viruses_infecting_class = container.getVirusesInfectingSpeciviedGroup(sorted_cls_description_items[0][1],
                                                                                  sorted_cls_description_items[0][0])
            # pdb.set_trace()
...
if __name__ == '__main__':
    ...
    if not (
                args.classifier_type.lower() == 'svm' or args.classifier_type.lower() == 'knn' or args.classifier_type.lower() == 'qda'):
    ...
    attributes, classes, ids, features = getAttributesClassesIdsFeatures(attribs, sequences, {0: {'Eukaryota': 1},
                                                                                              1: {'Bacteria': 1,
                                                                                                  'Archaea': 1}})
